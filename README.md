...
**页面版式**：**左列表 / 右详情**两栏 —— 左边是解析结果表（**只留 4 列**：预览 / 文件名 / 尺寸 / 操作 —— 每行都有「详情」按钮，鼠标悬停看完整路径，其余信息全在右边），右边固定显示选中那张的 **大图（上）+ 文案详情（下，可滚动）**；没读到参数的图，右栏照样出大图，模型 / 采样参数 / 提示词各栏显示「（没有…）」占位，不会空一块；右栏吸顶跟着看，点左边任意一行就换：
