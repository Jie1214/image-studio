...
**页面版式**：**左列表 / 右详情**两栏 —— 左边是解析结果表（**6 列**：预览 / 文件名 / 尺寸 / ComfyUI / 体积 / 操作 —— 每行都有「详情」按钮，鼠标悬停看完整路径，其余信息全在右边），右边固定显示选中那张的 **大图（上）+ 文案详情（下，可滚动）**；没读到参数的图，右栏照样出大图，模型 / 采样参数 / 提示词各栏显示「（没有…）」占位，不会空一块；右栏吸顶跟着看，点左边任意一行就换：
...
| 3. 看预览 | 左边列表：每张图归到哪一类、判定依据（ComfyUI tEXt prompt / EXIF / A1111 parameters…）、体积、尺寸；右边固定面板：**上=大图，下=判定依据与完整参数** |
...
### 图库索引与搜索（跨目录找图）
「读图参数」解析完一批后点 **＋ 存入图库**，结果会累积进 `work/library.sqlite`（SQLite，纯 stdlib；有 FTS5 就用全文索引，没有就 LIKE 兜底）。之后在搜索框里可以：

- **按提示词找图**：`marble`、`cinematic light`…（词与词之间是「都要包含」）
- **按模型 / LoRA 找图**：`krea2`、`detail_tweaker` —— 直接回答「这个模型我出过哪些图」
- **按工具过滤**：ComfyUI / NovelAI / SwarmUI / InvokeAI / A1111 系

搜索结果**直接用索引里的参数渲染右栏详情**（不用重新读原图），所以图被移走也能看参数；点「退出搜索」回到当前列表。状态行显示「图库：N 张（带提示词 M · ComfyUI K）· 工具分布 · 更新时间」。**清空图库只删索引，不动任何图片文件。**

回归测试：`.venv/Scripts/python.exe tests/test_library.py`（写入/搜索/过滤/upsert/统计/清空，在临时库里跑）。

### 上传缓存（work/uploads）会不会一直变大
两条输入路径，只有一条写盘：

- **浏览器「拖入 / 选择图片 / 选择文件夹」** → 浏览器拿不到本地绝对路径，只能把内容 POST 上来，服务端先复制一份到 `work/uploads/<时间戳>/` 再解析。所以会占空间（**原件在你自己的文件夹里，不动**）。
- **「读取该目录 / 分类目录」填路径** → 服务端直接读盘，**不复制、不占缓存**；想完全不占缓存就用这个入口。

自动清理（`config.json` 可调）：`upload_keep_hours`（默认 24 小时，更旧的批次删掉）、`upload_max_gb`（默认 5 GB，超了从最旧的批次删）—— 启动时执行，**最新一批永远保留**（可能正被界面引用），启动窗口会打印当前占用与策略。

手动清理：读图参数 → 图片来源卡片下方的「清理 24 小时前的 / 只留最新一批」。
回归测试：`.venv/Scripts/python.exe tests/test_cache_prune.py`（在临时目录里跑，不碰真实缓存）。
